...
class DCEL:
    ...
    def naive_polygon(self, vertices, out_face, name=None, make_in_face=True):
        """
...
class Vertex:
    ...
    def __repr__(self):
        try:
            return f"{self.name}  ({self.x:.3f},  {self.y:.3f})  {self.incident_edge.name}"
...
class Face:
    ...
    def __repr__(self):
        try:
            n = "nil"
            s = f"{self.name:3}  {self.outer.name if self.outer is not None else n:5}"
